...
namespace HallPass
{
    public static class ServiceCollectionExtensions
    {
        public static IServiceCollection AddHallPass(this IServiceCollection services, Action<HallPassOptions> config)
        {
            ...
            {
                services
                    .AddHttpClient(Options.DefaultName, c => c.Timeout = System.Threading.Timeout.InfiniteTimeSpan)
                    .AddHttpMessageHandler(serviceProvider => new HallPassMessageHandler(serviceProvider, options));
            }

            services
                .AddHttpClient(Constants.DEFAULT_HALLPASS_HTTPCLIENT_NAME, c => c.Timeout = System.Threading.Timeout.InfiniteTimeSpan)
                .AddHttpMessageHandler(serviceProvider => new HallPassMessageHandler(serviceProvider, options));
